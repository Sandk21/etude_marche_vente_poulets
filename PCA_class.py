...
class PCA_Analyse():
    ...
    def individuals_plot(self, plan_x: int = 1, plan_y: int = 2) -> plt.plot:
        ...
        for i in range(self.n):
            plt.annotate(self.X.index[i], (c[i, 0], c[i, 1]))
    ...
    def correlation_circle_plot(self, plan_x=1, plan_y=2) -> plt.plot:
        ...
        for j in range(self.p):
            plt.annotate(
                self.X.columns[j], (self.corr_var[j, 0], self.corr_var[j, 1]))
        ...
        for x, y in zip(self.corr_var[:, 0].tolist(), self.corr_var[:, 1].tolist()):
